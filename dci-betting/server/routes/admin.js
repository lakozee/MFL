...
const { scrapeCompetitionList, scrapeCompetitionScores } = require('../utils/dciScraper');

// Apply adminAuth to all routes in this file
router.use(adminAuth);

// ─── Helper: recalculate corps averages from competition_scores ───────────────
async function recalculateCorpsAverages() {
    await db.query(`
        UPDATE corps_stats cs_outer
        SET
          avg_brass = sub.avg_brass,
          avg_percussion = sub.avg_percussion,
          avg_guard = sub.avg_guard,
          avg_ge = sub.avg_ge,
          avg_visual = sub.avg_visual,
          total_score = sub.avg_brass + sub.avg_percussion + sub.avg_guard + sub.avg_ge + sub.avg_visual,
          competitions_count = sub.comp_count,
          updated_at = NOW()
        FROM (
          SELECT
            cs.corps_name,
            ROUND(AVG(cs.brass)::numeric, 2) as avg_brass,
            ROUND(AVG(cs.percussion)::numeric, 2) as avg_percussion,
            ROUND(AVG(cs.guard)::numeric, 2) as avg_guard,
            ROUND(AVG(cs.ge)::numeric, 2) as avg_ge,
            ROUND(AVG(cs.visual)::numeric, 2) as avg_visual,
            COUNT(*) as comp_count
          FROM competition_scores cs
          JOIN competitions c ON cs.competition_id = c.id
          WHERE c.season = 2025
          GROUP BY cs.corps_name
        ) sub
        WHERE cs_outer.corps_name = sub.corps_name AND cs_outer.season = 2025
    `);
}
        ...
        for (const comp of competitions) {
            try {
                ...
                const upsert = await db.query(
                    `INSERT INTO competitions (name, date, location, season, source_url)
                     VALUES ($1, $2, $3, $4, $5)
                     ...
                     RETURNING id`,
                    [comp.name, comp.date, comp.location, comp.season, comp.source_url]
                );
